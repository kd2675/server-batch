...
public class CoinJob {
    ...
    public Job delCoinJob(JobRepository jobRepository, @Qualifier(CoinStep.DEL_COIN_STEP) Step coinDeleteStep) {
        return new JobBuilder(DEL_COIN_JOB, jobRepository)
                .start(coinDeleteStep)
                .build();
    }
    @Bean(name = SEND_COIN_JOB)
    public Job sendCoinJob(JobRepository jobRepository,
